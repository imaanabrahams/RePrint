  ...
  <div class="auth fade-up">
    <div class="auth-card">
      ...
      <p v-if="show" class="notice" role="status">
        Account created successfully for <strong>{{ form.name }}</strong>. (Demo only — no backend connected.)
      </p>

      <form @submit.prevent="submit" class="form">
        <label>
          <span>Full name</span>
          <input v-model="form.name" class="input-field" placeholder="Jane Doe" required />
        </label>
        ...
        <label>
          <span>Password</span>
          <input v-model="form.password" type="password" class="input-field" placeholder="Create a password" required />
        </label>
      ...
      <div class="auth-links">
        ...
        <RouterLink to="/login" class="btn btn-accent login-link">Log in</RouterLink>
      </div>
    </div>
  </div>
</template>
