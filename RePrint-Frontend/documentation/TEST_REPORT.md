...
|------|---------|----------|---------|---------|
| **All files** | **42.11** | **39.73** | **41.43** | **42.25** |
| src (app shell + api) | 67.53 | 76.66 | 52.38 | 68.05 |
...
> Note: The coverage totals also include legacy scaffold files that came in with the repo restructure (`App.vue`, `main.js`, `src/router/index.js`, `src/services/api.js`, `src/stores/counter.js` and the duplicate lower-case views). These are unreferenced by `src/mainReprint.js` and sit at 0% — excluding them would put the real app close to 46% overall.
